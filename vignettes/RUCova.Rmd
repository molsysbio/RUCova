...
corr_cal33 <- t(assay(sce_Cal33,"counts")) |> 
  as.tibble() |>
...
sce_Cal33 <-  RUCova::rucova(sce = sce_Cal33, #can be a SingeCellExperiment class or a data.frame with cells in rows and features in columns (including col_name_sample and cell_id)
                                markers = m,
                                SUCs = x, 
...
heatmap_compare_corr(lower =  t(assay(sce_Cal33,"counts")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData) |> 
                                mutate_at(vars(m,x), asinh) |> 
                                group_by(dose) |>
                                mutate(across(all_of(x), ~ .x - mean(.x))) |> #as center_SUCs = "per_sample",
                                ungroup() |> 
                                select(m,x) |> 
                                cor(method= "pearson") ,
                     
                     upper = t(assay(sce_Cal33,"counts_rucova_per_sample")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData) |> 
                                mutate_at(vars(m,x), asinh) |> 
                                group_by(dose) |>
...
```{r, fig.height=10, fig.width=10}
t(assay(sce_Cal33,"counts")) |>  as.tibble() |> cbind(sce_Cal33@colData) |> 
  ...
  rbind(
    ...
    t(assay(sce_Cal33,"counts_rucova_per_sample")) |>  as.tibble() |> cbind(sce_Cal33@colData)  |> 
    mutate_at(vars(x,m), asinh) |> 
    pivot_longer(names_to = "marker", values_to = "value", c(x,m)) |> 
    group_by(marker) |> 
    summarise(logFC = mean(value[dose=="10Gy"])-mean(value[dose=="0Gy"])) |> 
...
sce_Cal33 <-  RUCova::rucova(sce = sce_Cal33,  #SingleCellExperiment object
                                markers = m,
                                SUCs = x, 
                                apply_asinh_SUCs = TRUE, #when taking surrogates we need to asinh-transform them
                                model = "simple",
                                center_SUCs = "across_samples",
                                name_output = "rucova_across_samples")
```
...
heatmap_compare_corr(lower = t(assay(sce_Cal33,"counts")) |> 
                     ...
                     upper = t(assay(sce_Cal33,"counts_rucova_across_samples")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData) |> 
                                mutate_at(vars(m,x), asinh) |> 
                                select(m,x) |> 
...
t(assay(sce_Cal33,"counts")) |>  as.tibble() |> cbind(sce_Cal33@colData) |> 
  ...
  t(assay(sce_Cal33,"counts_rucova_across_samples")) |>  as.tibble() |> cbind(sce_Cal33@colData) |> 
    mutate_at(vars(x,m), asinh) |> 
    pivot_longer(names_to = "marker", values_to = "value", c(x,m)) |> 
    group_by(marker) |> 
    summarise(logFC = mean(value[dose=="10Gy"])-mean(value[dose=="0Gy"])) |> 
...
```{r echo=FALSE}
sce_Cal33 <-  RUCova::rucova(sce = sce_Cal33, # marker intensity values in linear scale
                                markers = m,
...
heatmap_compare_corr(lower = t(assay(sce_Cal33,"counts")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData) |> 
                                cbind(reducedDim(sce_Cal33,"PCA")) |> 
                                mutate_at(vars(m,x), asinh) |>
                                select(m,x,"PC1","PC2","PC3","PC4") |> 
                                cor(method= "pearson") ,
                     
                     upper = t(assay(sce_Cal33,"counts_rucova_pc1")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData) |> 
                                cbind(reducedDim(sce_Cal33,"PCA")) |> 
                                mutate_at(vars(m,x), asinh) |>
...
```{r, fig.height=10, fig.width=10}
  ...
  rbind(
    ...
    t(assay(sce_Cal33,"counts_rucova_pc1")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData)  |> 
    mutate_at(vars(x,m), asinh) |> 
    pivot_longer(names_to = "marker", values_to = "value", c(x,m)) |> 
    group_by(marker) |> 
    summarise(logFC = mean(value[dose=="10Gy"])-mean(value[dose=="0Gy"])) |> 
    mutate(data = "simple PC1, across samples")
...
sce_Cal33@metadata$rucova_across_samples$adjr2 |> mutate(model = "simple all, across samples") |> 
  rbind(sce_Cal33@metadata$rucova_per_sample$adjr2|> mutate(model = "simple all, per sample")) |> 
      rbind(sce_Cal33@metadata$rucova_pc1$adjr2 |> mutate(model = "simple PC1, across sample")) |> 
...
sce_Cal33@metadata$rucova_across_samples$stand_slopes |> mutate(model = "simple all, across samples") |> 
  rbind(sce_Cal33@metadata$rucova_per_sample$stand_slopes |> mutate(model = "simple all, per sample")) |> 
    rbind(sce_Cal33@metadata$rucova_pc1$stand_slopes |> mutate(model = "simple PC1, across sample")) |> 
...
Here we just show one example: Let's say different samples (dose) within Cal33 are different batches, and we would like to adjust for it in addition to removing unwanted correlations with all SUCs. Then: 
...
t(assay(sce_Cal33,"counts")) |> 
  ...
  rbind(
    ...
    t(assay(sce_Cal33,"counts_offset_per_sample")) |> 
                                as.tibble() |>
                                cbind(sce_Cal33@colData)  |> 
    mutate_at(vars(x,m), asinh) |> 
    pivot_longer(names_to = "marker", values_to = "value", c(x,m)) |> 
...
```{r, fig.height=5, fig.width=10}
sce_Cal33@metadata$offset_per_sample$eff_coefficients |> 
...
```{r}
sce <-  RUCova::rucova(sce = sce, # marker intensity values in linear scale
                                markers = m,
                                SUCs = x, 
                                apply_asinh_SUCs = TRUE, #when taking surrogates we need to asinh-transform them
                                model = "interaction",
...
```{r, fig.height=10, fig.width=10}
sce@metadata$interaction$eff_coefficients |> 
  filter(surrogate != FALSE) |> 
  ggplot(aes(x = eff_value, y = marker, fill = sample)) + 
  geom_col(position = "dodge") +
