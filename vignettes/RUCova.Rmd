...
remotes::install_github("molsysbio/RUCova@vignette-update", force = TRUE)
```
...
```{r, eval = FALSE}
data_Cal33 |> mutate(PC1 = -PC1) #variable not saved as not necessary here
```
...
simple_all_x_across$adjr2 |> mutate(model = "simple all, across samples") |> 
  rbind(simple_all_x_persample$adjr2 |> mutate(model = "simple all, per sample")) |> 
    rbind(simple_pc1_x$adjr2 |> mutate(model = "simple PC1, across sample")) |> 
...
simple_all_x_across$stand_slopes |> mutate(model = "simple all, across samples") |> 
  rbind(simple_all_x_persample$stand_slopes |> mutate(model = "simple all, per sample")) |> 
    rbind(simple_pc1_x$stand_slopes |> mutate(model = "simple PC1, across sample")) |> 
...
```{r}
offset_all_x_persample <-  RUCova::rucova(data = data_Cal33, # marker intensity values in linear scale
                                markers = m,
                                SUCs = x, 
                                apply_asinh_SUCs = TRUE, #when taking surrogates we need to asinh-transform them
                                model = "offset",
...
data_reg_offset_all_persample <- offset_all_x_persample$data_reg #output of marker intensity values are also in linear scale, linear in -> linear out
```
...
```{r, fig.height=5, fig.width=10}
offset_all_x_persample$eff_coefficients |> 
...
```{r}
intercept_all_x_across <-  RUCova::rucova(data = data, # marker intensity values in linear scale
                                markers = m,
                                SUCs = x, 
                                apply_asinh_SUCs = TRUE, #when taking surrogates we need to asinh-transform them
                                model = "interaction",
...
data_intercept_all_x_across <- intercept_all_x_across$data_reg #output of marker intensity values are also in linear scale, linear in -> linear out
```
...
```{r, fig.height=10, fig.width=10}
intercept_all_x_across$eff_coefficients |> 
  filter(surrogate != FALSE) |> 
  ggplot(aes(x = eff_value, y = marker, fill = sample)) + 
  geom_col(position = "dodge") +
