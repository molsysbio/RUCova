---
title: "RUCova: Removal of Unwanted Covariance in mass cytometry data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{RUCova's vignette}
...
## Install RUCova and call dependencies
...
data: A tibble containing mass cytometry data of single-cell marker signals [rows = cells, columns = markers and metadata] in linear scale. This data set should be clean, meaning you excluded beads, debris, doublets, dead cells, and single-cells are demultiplexed.
...
x = "PC1" # or c("PC1","PC2"), or all PCs, which is equivalent as taking all four SUCs.
```
...
Across samples: SUCs are zero-centered across all samples before applying the linear model. This is recommended when the relationship (e.g.: logFC) between samples is an unwanted covariance and it is thought to be affected by cell size or staining efficiency.
...
Per sample (more conservative): SUCs are zero-centered per sample applying the linear model. This is recommended when the relationship (e.g.: logFC) between samples is a desired covariance and is not affected by cell size or staining efficiency.
...
<!-- Let's imagine you want to be conservative and only remove correlations between markers and PC1 (of SUCs). Then, `SUCs= "PC1"` and `apply_asinh_SUCs = FALSE`, as asinh transformation is not necessary on PCs (it was applied on SUCs before PCA). This applies to all models. -->
...
<!--                                 model = "simple", -->
<!--                                 center_SUCs = "across_samples",#PCA was done across the whole data set -->
